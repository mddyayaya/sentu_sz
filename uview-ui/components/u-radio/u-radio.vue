<template>
	<view class="u-radio" :style="[radioStyle]" :class="{checkbg:isChecked==true}">
...
<script>
	...
	export default {
		...
		computed: {
			...
			// 组件的勾选图标的尺寸，默认20
			elIconSize() {
				return this.iconSize ? this.iconSize : (this.parentData.iconSize ? this.parentData.iconSize : 10);
			},
			// 组件选中激活时的颜色
			elActiveColor() {
...
<style lang="scss" scoped>
	...
	.u-radio {
		...
		&__icon-wrap {
			...
			&--checked {
				color: #fff;
				background-color: #CE2741 !important;
				border-color: #CE2741 !important;
			}
